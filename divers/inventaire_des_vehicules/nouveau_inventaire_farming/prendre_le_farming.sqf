...
if (lbtext ([1500,0]) == "Rien dans l'inventaire.") exitwith {hint "Rien n'est selectionné ...";};
...
_nom_du_produit_de_farming = liste_du_farming_colonne_de_droite select ligne_colone_du_farming_droite select 2; 
_nom_de_la_variable_du_farming = liste_du_farming_colonne_de_droite select ligne_colone_du_farming_droite select 0;
_nombre_de_place_de_la_variable = liste_du_farming_colonne_de_droite select ligne_colone_du_farming_droite select 1; 

_espace_de_l_objet_du_farming = player getvariable "usedspace";
_espace_total_de_l_objet_du_farming = player getvariable "totalspace";
...
if ((_espace_de_l_objet_du_farming + (choix_montant_du_menu_farming * _nombre_de_place_de_la_variable)) > _espace_total_de_l_objet_du_farming) exitwith {hint format ["Opération impossible, il manque %1 place dans votre inventaire ...", _calcul_pour_resultat_place_disponible];};

_condition_quantite_suffisante = true;
if (typeName (objet_du_farming_a_scanner getvariable _nom_de_la_variable_du_farming) == "BOOL") then 
{
...
{
	if (objet_du_farming_a_scanner getvariable _nom_de_la_variable_du_farming < choix_montant_du_menu_farming) then
	{
		_condition_quantite_suffisante = false;
		_message = format ["Opération impossible, %1 %2 maximum ...", objet_du_farming_a_scanner getvariable _nom_de_la_variable_du_farming, _nom_du_produit_de_farming];
	};		
};
...
if (typeName (objet_du_farming_a_scanner getvariable _nom_de_la_variable_du_farming) == "BOOL") then
{
	player setvariable [_nom_de_la_variable_du_farming, true, true];
	objet_du_farming_a_scanner setvariable [_nom_de_la_variable_du_farming, nil, true];
}else
{
...
hint format ["Vous avez prit %1 %2.", choix_montant_du_menu_farming, _nom_du_produit_de_farming];
