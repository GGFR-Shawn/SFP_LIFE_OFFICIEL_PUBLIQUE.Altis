...
if (!isnil {player getvariable "licence_gang_v2"}) then 
{
	if (checkpointgang == 0) then
	{
		...
		[[[],"divers\gendarmerie\mission_de_patrouille\joueur_son_cp_2.sqf"],"BIS_fnc_execVM",true,false] spawn BIS_fnc_MP;
		checkpointgang = 1;
		_start = time;
		waitUntil {player distance (getmarkerpos "CheckPoint2") > 30};
		_finish = time - _start;
		_pay = ceil(_finish * 5) / 1.5;
