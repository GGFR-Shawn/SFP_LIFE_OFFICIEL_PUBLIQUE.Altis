...
// Modif By SFP Maxou - www.sfpteam.fr 04052014

#define MAX_SPEED_WHILE_FASTROPING 10
#define MAX_SPEED_ROPES_AVAIL 30


#define STR_TOSS_ROPES "Déployer Corde"
#define STR_FAST_ROPE "Descendre à la corde"
...
zlt_rope_helis = ["O_Heli_Light_02_unarmed_F","O_Heli_Light_GIGN_SFP","I_Heli_light_GIGN_SFP","B_Heli_Transport_GIGN_SFP"];
zlt_rope_helidata = 
[
	[
		["O_Heli_Light_02_unarmed_F", "O_Heli_Light_02_F","O_Heli_Light_GIGN_SFP"],
		[1.35,1.35,-24.95],
		[-1.45,1.35,-24.95]
	],
	[
		["B_Heli_Transport_01_F", "B_Heli_Transport_01_camo_F","B_Heli_Transport_GIGN_SFP"],
		[-1.11,2.5,-24.7],
		[1.11,2.5,-24.7]
	],
	[
	...
	[
		["B_Heli_Light_01_F","Heli_Light_Civil_SFP","Heli_Light_Gendarme_SFP"],
		[0.6,0.5,-25.9],
		[-0.8,0.5,-25.9]
	],
	[
		["I_Heli_light_GIGN_SFP"],
		[0.6,2.5,-25.9],
		[-0.8,2.5,-25.9]
	]
];
...
zlt_fnc_fastropeaiunits = {
		...
		diag_log ["Diag SFP : zlt_fnc_fastropeaiunits", _this];
...
zlt_fnc_fastrope = {
	diag_log ["Diag SFP : fastrope", _this];
...
zlt_fnc_fastropeUnit = {
	...
	_unit setPosATL [(getPosATL _unit select 0), (getPosATL _unit select 1), 0 max ((getPosATL _unit select 2) - 3)];
	while {alive _unit and (getPosATL _unit select 2) > 1 and (abs (speed _heli)) < MAX_SPEED_WHILE_FASTROPING  and _zc > -24} do {
