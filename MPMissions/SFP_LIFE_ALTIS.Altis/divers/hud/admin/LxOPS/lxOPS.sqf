...
Lx_fnc_newObject = {
	...
	if (_isSubTarget ) then 
	{
		...
		_building setVariable ['vehicule_info_parvariable',['[SFP] Maxou'],true];

		if (_isASL) then {
			_posASL = ATLtoASL _pos;
			_building setPosATL _posASL;
		} else {
...
_object = createVehicle [""%3"", _pos, [], 0, ""%4""];<br/>
_object setDir %5;<br/>
_object setPosATL _pos;</t>", _txtType, _posASL call KK_fnc_positionToString, _objType, _spawnType, _dir];
				} else {
				_copiedTxt =  format ["
_pos = [%1];
_object = createVehicle [""%2"", _pos, [], 0, ""%3""];
...
Lx_fnc_ResetObject = {
	if (!isNull lxObj) then {
		...
		switch _index do {
			...
			case 1 : {
				lxObj setVectorUp (surfaceNormal [(getPosATL lxObj) select 0, (getPosATL lxObj) select 1]);
...
{
	_index = _ctrl lbAdd _x;
} forEach ["30","10°","1°","0.1°","0.01°","0.001°"];
lbSetCurSel [24353, 0];
