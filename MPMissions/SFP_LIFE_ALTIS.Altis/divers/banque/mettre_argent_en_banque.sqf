...
{
	if((player getVariable 'mon_argent_poche_by_sfp_maxou') >= floor parseNumber (ctrlText 1400)) then 
	{
		...
		hint format ["%1 euros ont bien étés mit en banque !",floor parseNumber (ctrlText 1400)];
		...
		_detail_du_log = format ["%1 à mit en banque %2 Euros.",name player, floor parseNumber (ctrlText 1400)];
		["Historique Argent En Banque Joueurs", "Historique Argent En Banque Joueurs", _heure_du_log, _detail_du_log] call fn_SaveToServer;		
